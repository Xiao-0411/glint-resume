# sgjl.cloud 部署说明（备选方案，当前未使用）

> **注意：这份文档描述的不是当前线上架构。**
>
> 当前实际架构：前端在 **Cloudflare Pages**（纯静态，没有 `/api` 反代），
> 后端经 **Cloudflare Tunnel** 暴露为 `api.sgjl.cloud`，前端跨域调用它。
> 见 `运行指南.md` 与 `frontend/.env.production`。
>
> 下面这套是「自购 VPS + 公网 IP + Nginx 同源反代」的备选路线，
> 需要公网 A 记录和自建证书。除非确定要迁到自托管，否则不要照做——
> 尤其不要按这里的同源假设去改前端 API 地址，会直接导致线上登录失败。
